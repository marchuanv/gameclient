function CapturePlayerScene(sceneManager, sceneObjBuilder, sceneSelector, sceneEventManager, phaserGame, playerScoreManager){
	this.create = function(){
		...
		sceneEventManager.subscribeToAnimationComplete(function(animation, sceneConfig){
			if (sceneConfig.name.indexOf("submit") >= 0){
				sceneObjBuilder.getText(function(inputText){
						playerScoreManager.createPlayerScore(inputText.text);
						sceneSelector.next();
						phaserGame.reset();
				});
			}
...
module.exports=CapturePlayerScene;