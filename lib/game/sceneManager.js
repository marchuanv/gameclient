...
this.playAllAnimationsForSceneObj = function(sceneObj, animNameFilter){
	sceneObjBuilder.getAnimations(function(scene){
		enumerate(sceneObj.animations, this, function item(indexConfig){
			if (scene.config.name == indexConfig.name || (animNameFilter && indexConfig.name.indexOf(animNameFilter) >= 0) ){
				playAnimationForSceneObj(scene.obj, 
					scene.config,
					scene.state);
			}
