function DiagramCanvas() {
	...
	this.drawNodes = function(_nodes){
		for (var i = _nodes.length - 1; i >= 0; i--) {
			var node = _nodes[i];
			var nodeA = new Node(node.x, node.y, node.Id);
	...
	function SelfLink(node, mouse, Id) {
		this.node = node;
		this.anchorAngle = 0;
	...
	function StartLink(node, start, Id) {
		this.node = node;
		this.deltaX = 0;
		this.deltaY = 0;
		this.text = '';
	...
	canvas.onmousedown = function(e) {
		...
		if(selectedObject != null) {
			...
			} else if(selectedObject instanceof Node || selectedObject instanceof Link || selectedObject instanceof TemporaryLink){
				movingObject = true;
				deltaMouseX = deltaMouseY = 0;
				console.log("DOWN");
				if (stateMachineInstance.selectedNode && selectedObject instanceof Node){
					stateMachineInstance.selectedNode.call(stateMachineInstance, selectedObject);
				}
			}
			resetCaret();
		} else if(shift) {
			...
			stateMachineInstance.nodeLostFocus.call(stateMachineInstance);
		}
		draw();
	...
	canvas.onmousemove = function(e) {
		...
		if(movingObject) {
			...
			if(selectedObject instanceof Node) {
				...
				if (stateMachineInstance.nodeMoveStart){
					stateMachineInstance.nodeMoveStart.call(stateMachineInstance);
				}
			}
			draw();
		}
	...
	canvas.onmouseup = function(e) {
		if (movingObject && selectedObject instanceof Node && stateMachineInstance.nodeMoveEnd){
			stateMachineInstance.nodeMoveEnd.call(stateMachineInstance);
		}
		movingObject = false;
		if(currentLink != null) {
			if(!(currentLink instanceof TemporaryLink)) {
				...
				if (stateMachineInstance.linkCreated){
					if (selectedObject.nodeA || selectedObject.nodeB || selectedObject.node){
						stateMachineInstance.linkCreated.call(stateMachineInstance, currentLink);
					}
				}
				resetCaret();
			}
	...
	document.onkeydown = function(e) {
		...
		if(key == 16) {
			...
			if(selectedObject != null) {
				...
				if (stateMachineInstance.nodeDeleted){
					if (selectedObject.nodeA || selectedObject.nodeB || selectedObject.node){
						return;
					}
					stateMachineInstance.nodeDeleted.call(stateMachineInstance, selectedObject);
				}
				selectedObject = null;
				draw();
			}
