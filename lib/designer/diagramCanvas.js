function DiagramCanvas() {
	...
	canvasPos.deltaX = center_x;
	canvasPos.deltaY = center_y;
	var mouseX = 0;
	var mouseY = 0;
	...
	canvas.onmousedown = function(e) { // Are for links ONLY
		...
		if(shift) {
			...
			if (selectedObject != null){
				...
				mouseX = (e.clientX - r.left) - canvasPos.deltaX;
				mouseY = (e.clientY - r.top) - canvasPos.deltaY;

				var x = e.clientX - r.left;
			    var y = e.clientY - r.top;
			    canvasPos.deltaX = (x - mouseX); // total distance in x
				canvasPos.deltaY = (y - mouseY); // total distance in y

				diagramRendering.update(canvasPos.deltaX, canvasPos.deltaY);
			}
			diagramRendering.resetCaret(function(){
				diagramRendering.render();
			});
	...
	canvas.onmousemove = function(e) {
		...
		if(selectedObject && selectedObject.isMoving) {
			...
			diagramRendering.render();
		}else if (selectedObject == null && panMap){
			var r = canvas.getBoundingClientRect();
			var x = e.clientX - r.left;
		    var y = e.clientY - r.top;
		    canvasPos.deltaX = (x - mouseX); // total distance in x
