...
this.create = function(){
		...
		if (sceneConfig.name.startsWith("Node")){
			setTimeout(function(){
				sceneObjBuilder.getSprites(function(obj, config, state){
					if (config.name == "contextLoader" && state.cancelled == false){
						obj.visible = true;
						phaserGame.world.bringToTop(obj);
						obj.x = mouse.x;
						obj.y = mouse.y;
						sceneManager.playAllAnimationsForSceneObj(config, ["load"]);
					}
				});
			},1000);
		}
	...
	sceneEventManager.subscribeToMouseDown(function (mouse){
		sceneObjBuilder.getSprites(function(obj, config, state){
			if (config.name == "contextLoader"){
	...
	sceneEventManager.subscribeToMouseUp(function (){
		sceneObjBuilder.getSprites(function(obj, config, state){
			if (config.name == "contextLoader"){
