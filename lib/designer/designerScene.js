...
this.create = function(){
	...
	sceneEventManager.subscribeToSpriteDragStop(function (mouse, obj, sceneConfig, state){
		...
		if (_sceneConfig.name.endsWith("NodeTemplate")){
			...
			getNodeLink(_sceneConfig.name, function(_link, _linkConfig){
				_linkConfig.xPosition = mouse.x;
				_linkConfig.yPosition = mouse.y;
				drawLink(_sceneConfig.name, _linkConfig.xPosition, _linkConfig.yPosition);
			});
		}
		cache.set("pan",{});
	});
	...
	sceneEventManager.subscribeToSpriteSelected(function(mouse, obj, sceneConfig, state){
		...
		cache.get("disconnectedLink",function(_linkConfig){
			...
			phaserGame.reset(function(){
				drawLink(_linkConfig.name, _linkConfig.xPosition, _linkConfig.yPosition);
			});
		});
	});
...
function drawLink(linkName, x, y){
	...
	getLinkNodes(linkName, function(_node, _nodeConfig, _link, _linkConfig, isStartLink){
		...
		_link.x = endX;
		_link.y = endY;
		if (isStartLink){ //STARTING NODE
			startNode =_node;
			radius = _nodeConfig.radius;
			...
			angle =  phaserGame.physics.arcade.angleBetween({x: startX ,y: startY }, _node);
			endX = _node.x-50; + radius * Math.cos(angle);
			endY = _node.y-50; + radius * Math.sin(angle);
			radius = _nodeConfig.radius;
			_link.rotation =  angle;

			// adjustLinkPos = setInterval(function(){
				// isLinkOverlapNode(linkName,function(){
				// 	endX--;
				// 	endY--;
				// },function (){
