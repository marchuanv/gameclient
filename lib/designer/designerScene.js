...
function startLinkDraw(startNode, startNodeConfig, endNode, endNodeConfig, link, linkConfig, isMouseOver, mouse){
	...
	if (endNode){
		if (startNodeConfig.shape == "circle"){
			...
			var startX = startNode.x + startNodeConfig.radius * Math.cos(startAngle);
			var startY = startNode.y + startNodeConfig.radius * Math.sin(startAngle);
			var endAngle =  phaserGame.physics.arcade.angleBetween(endNode, startNode);
			var endX = endNode.x + endNodeConfig.radius	 * Math.cos(endAngle);
			var endY = endNode.y + endNodeConfig.radius	 * Math.sin(endAngle);
			setConfigPositions(endX, endY, startAngle, link, linkConfig);
		...
		if (startNodeConfig.shape == "rectangle"){
			...
			var startNodePositions = getSquareNodePositions(startNode);
			var left = startNodePositions.left;
			var right = startNodePositions.right;
			var top = startNodePositions.top;
			var bottom = startNodePositions.bottom;
