			...
			if (!isDisconnectedLink){
				if (!isStartNode){
					...
					factory.phaserGame(function(_instance){
						_link.x = x;
						_link.y = y;
						// if (_linkNodeConfig.shape == "circle"){
							//var distanceOffset = radius*2;
							...
							var loop = setInterval(function(){
								...
								if (_linkNode.overlap(_link)){
									_link.width--;
								}else{
									clearInterval(loop);
								}
							},300);
							// _link.width -= distanceOffset;
						// }
						// if (_linkNodeConfig.shape == "rectangle"){
						// 	var phaserPoint = new Phaser.Point(_link);
						// 	while(!_linkNode.overlap(_link)) {
						// 		_link.width +=100;
						// 		angle = _link.rotation = phaserGame.physics.arcade.angleBetween(_link, _linkNode);
						// 		_link.rotation = angle;
						// 	}
