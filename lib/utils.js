...
var _timerCache = new cache();

function thread(cbStart){
	...
	window.postMessage({ start: true },url);
};

function timedCallback(seconds, Id, cbComplete){
	var end = false;
	var url = "http://"+window.location.host;
	return {
		start:function(){
			window.addEventListener("message", function(e){
				if (e.data.waiting == false){
					setTimeout(function(){
						end = true;
						cbComplete();
					},seconds*1000);
		...
		stop: function(){
			_timerCache.set(Id, "true");
		},
		wait: function(){
			window.postMessage({ waiting: false },url);
			while(end == false){
				var count = 0;
				while(count < 100000000){
					count++;
				};
				window.postMessage({ waiting: true },url);
			};
		}
	};
};
